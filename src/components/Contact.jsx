...
function Contact() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {             // Send a POST request to the server with the form data             
            ...
            const data = await response.json(); if (response.ok) {
                ...
                setFormData({
                    myName: "", myEmail: "", myReason: "", myComments: "",
                });
            }
            else {
    ...
    return (
        <main>
            ...
            <form onSubmit={handleSubmit}>
                ...
                <input type="text" id="myName" name="myName" value={formData.myName}
                    onChange={handleChange} // Call handleChange on field change                 
                />
                {/* Email field */}
                <label htmlFor="myEmail">E-mail:</label>
                <input type="text" id="myEmail" name="myEmail" value={formData.myEmail}
                    onChange={handleChange} // Call handleChange on field change                 
                />
                {/* Explanation:                     
                    - The `htmlFor` attribute in JSX is equivalent to the standard `for` attribute in HTML.                     
                    - In JSX, `for` is a reserved keyword in JavaScript, so React uses `htmlFor` instead.                     
                    - It connects the <label> tag to the input field with the matching `id`.                     
                    - For example, `htmlFor="myName"` links the label to the input field with `id="myName"`.                 
                    */}
                {/* Reason for contact with a dropdown list */}
                <label htmlFor="myReason">Reason for Contact:</label>
                <input type="text" id="myReason" name="myReason" list="reasons" // Connect to the datalist with id="reasons"                     
                    value={formData.myReason} onChange={handleChange} // Call handleChange on field change                     
                ...
                </datalist>                  {/* Comments field */}
                <label htmlFor="myComments">Comments:</label>
                <textarea rows="2" cols="20" id="myComments" name="myComments" value={formData.myComments}
                    onChange={handleChange} // Call handleChange on field change                
                ></textarea>
                {/* Submit button */}
